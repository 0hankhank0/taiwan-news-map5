...
function buildTdxConstructionUrl(source) {
  ...
  if (source.type === "City") {
    basePath.push("City", encodeURIComponent(source.path));
  } else if (source.type === "Highway") {
    basePath.push("Highway");
  } else {
    throw new Error(`Unsupported TDX Construction source type: ${source.type}`);
  }
...
async function loadStaticCmsCache(accessToken, startedAt) {
  const now = Date.now();
  if (now < tdxStaticCmsCache.backoffUntil) {
    return tdxStaticCmsCache.bySource;
  }

  if (now < tdxStaticCmsCache.expiresAt && tdxStaticCmsCache.bySource.size > 0) {
    return tdxStaticCmsCache.bySource;
  }
  ...
  tdxStaticCmsCache = {
    bySource,
    expiresAt: now + TDX_STATIC_CACHE_MS,
    backoffUntil: sawRateLimit ? now + TDX_BACKOFF_MS : 0,
  };
...
async function fetchTDXTrafficEvents(startedAt, preloadedToken = "") {
  try {
    ...
    const now = Date.now();
    if (now < tdxLiveCmsCache.backoffUntil) {
      return tdxLiveCmsCache.events;
    }

    if (now < tdxLiveCmsCache.expiresAt && tdxLiveCmsCache.events.length > 0) {
      return tdxLiveCmsCache.events;
    }

    let accessToken = preloadedToken;
    if (!accessToken && process.env.TDX_CLIENT_ID && process.env.TDX_CLIENT_SECRET) {
      try {
    ...
    const filteredEvents = trafficEvents.filter(
      (item) => Number.isFinite(item.lat) && Number.isFinite(item.lng)
    );
    tdxLiveCmsCache = {
      events: filteredEvents,
      expiresAt: now + TDX_LIVE_CACHE_MS,
      backoffUntil: sawRateLimit ? now + TDX_BACKOFF_MS : 0,
    };
...
async function fetchTDXConstructionEvents(accessToken, startedAt) {
  try {
    if (getRemainingTime(startedAt) < 1000) {
      return [];
    }

    const now = Date.now();
    if (now < tdxConstructionCache.backoffUntil) {
      return tdxConstructionCache.events;
    }

    if (now < tdxConstructionCache.expiresAt && tdxConstructionCache.events.length > 0) {
      return tdxConstructionCache.events;
    }
    ...
    const requests = TDX_CONSTRUCTION_SOURCES.map(async (source) => {
      ...
      try {
        ...
        const rawRecords = extractArrayFromTdxPayload(data);
        return rawRecords
          .map((item) => normalizeCmsConstructionRecord(item, source))
          .filter(Boolean);
      } catch (error) {
        const status = error.response?.status;
        if (status === 429) {
        ...
        console.warn(
          `[events] TDX construction failed for ${source.type}/${source.path}:`,
          status ? `HTTP ${status}` : error.message
        );
        return [];
      }
    ...
    console.error("[events] TDX construction fetch failed:", error.message);
    return [];
  }
}
  ...
  try {
    ...
    if (process.env.TDX_CLIENT_ID && process.env.TDX_CLIENT_SECRET) {
      try {
        ...
        console.warn("[events] Failed to get TDX token:", error.message);
      }
    }

    const [rssResults, tdxEvents, constructionEvents] = await Promise.all([
      Promise.all(DEFAULT_RSS_SOURCES.map((url) => fetchOneRssFeed(url, startedAt))),
      fetchTDXTrafficEvents(startedAt, sharedAccessToken),
      fetchTDXConstructionEvents(sharedAccessToken, startedAt),
    ]);
    ...
    console.log(
      "[events] rss=%d tdx=%d construction=%d ai=%d aiCached=%s rule=%d final=%d totalMs=%d",
      rssItems.length,
      tdxEvents.length,
