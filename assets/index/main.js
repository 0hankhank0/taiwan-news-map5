    ...
    function makeEventImpactRow(ev) {
        ...
        return `<div class="event-impact-row" aria-label="事件影響與建議">
            <span class="event-advice-chip"><i class="fa-solid fa-route"></i>${advice}</span>
            <span class="event-impact-chip"><i class="fa-solid fa-signal"></i>${status}</span>
            ${tags.map(tag => `<span class="event-impact-chip">${tag}</span>`).join("")}
        </div>`;
    }
    ...
    function updateCurationMeta(events){
        ...
        const cityLabel = isNearbyMode ? `附近 ${formatRadiusLabel(nearbyRadiusMeters)}` : (cityValue === "all" ? "全台" : cityValue);
    ...
    function buildEmptyStateHtml() {
        ...
        const filtered = activeCategory !== "all" || searchKeyword || currentCityFilter() !== "all" || isNearbyMode;
        const title = hasSourceEvents && filtered ? "沒有符合目前條件的事件" : "目前沒有可顯示的事件";
        const copy = hasSourceEvents && filtered
            ? (isNearbyMode ? `附近 ${formatRadiusLabel(nearbyRadiusMeters)} 目前沒有符合條件的事件，可放大半徑或查看全台。` : "試著放寬分類、城市或關鍵字，清單會立即重新整理。")
    ...
    function updateMobileFilterSummary(count = 0) {
        ...
        if (!summary) return;
        summary.innerHTML = `<span>目前篩選：${getCurrentCategoryLabel()}</span><strong>${count} 筆</strong>`;
    }
    ...
    function renderEvents(){
        ...
        setStatus(isNearbyMode ? `附近 ${formatRadiusLabel(nearbyRadiusMeters)}：${events.length} 筆事件` : `顯示 ${events.length} 筆事件`);
